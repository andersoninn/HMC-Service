...
const ServiceDescription = forwardRef<HTMLElement, {}>((_, ref): React.ReactElement | null => {
  const { selectedService } = useServiceStore();

  if (!selectedService) {
    return <p className="text-gray-500">Selecione um serviço para ver os detalhes.</p>;
  }

  const serviceDetails: Record<string, ServiceInfo> = {
    movingChange: {
      title: 'Especialistas em mudanças desafiadoras. Você relaxa, a gente resolve.',
      description:
    ...
    organizer: {
      title: 'Organização profissional para quem valoriza tempo, espaço e bem-estar.',
      description:
  ...
  const highlightMap: Record<string, string[]> = {
    movingChange: ['Especialistas', 'em', 'mudanças', 'a', 'gente', 'resolve'],
    cleaning: ['Limpeza', 'Rápido', 'discreto', 'confiável'],
    organizer: ['Organização', 'profissional', 'valoriza', 'tempo', 'espaço', 'bem-estar'],
  };
